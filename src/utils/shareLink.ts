 ...
 * 把画布 JSON（剥离图片二进制后）deflate 压缩 + base64url 编码进 URL 的 hash 段，
 * 形如 https://map.linkbrain.top/#import=<payload>。
 *
 * - hash 不会随请求发送到服务器，天然不经服务端存储/审核；
...
/** 生成 hash 分享链接；tooLarge = 链接过长，建议改用导出 JSON 文件 */
