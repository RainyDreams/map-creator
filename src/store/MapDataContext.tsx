...
import { scheduleExportPrewarm } from '@/utils/exportPrewarm'
...
function normalizeData(raw: unknown): MapData | null {
  ...
  return {
    ...
    customPosition:
      typeof d.customPosition === 'boolean'
        ? d.customPosition
    ...
    // v1.39 迁移：旧数据无 provinceColors 字段时回退空表；键为省份名、值须为 #rrggbb 或 'auto'（v1.40 起）
    provinceColors: (() => {
      ...
      for (const [k, v] of Object.entries(raw)) {
        ...
        if (v !== 'auto' && !/^#[0-9a-fA-F]{6}$/.test(v)) continue
        out[k] = v
...
export function MapDataProvider({ children }: { children: ReactNode }) {
  ...
  const setFontSlot = useCallback(
    (slot: FontSlot, fontId: string) => {
      // 字体变了，导出用的字体嵌入 CSS 缓存必须重算；闲时重新预热
