...
const changelog = [
  ...
  },
  {
    ...
    date: '2026-07-23',
    items: [
      '省份卡片位置三模式：一列 / 两列（自动布局）/ 自定义（手动摆放）；直接拖动卡片自动切到自定义模式，切回自动布局时手动位置保留但不生效',
      '位置一键重置：拖乱了可在「省份卡片位置」下点「重置位置」，清除全部手动偏移回到自动布局',
      '卡片点击置顶：点击或按住任意省份卡片，该卡片层级自动上移，不再被相邻卡片盖住',
      '画布防误触：画布上所有文字不可选中，图片与文字不可触发浏览器原生拖动',
      '字体加载提速：5 个子集字体改为稳定 URL 并行预载（preload），配合 CDN 缓存与 swap 渐进渲染，打开页面字体到达更快',
      '「每侧标注列数」更名为「省份卡片位置」，语义更清晰',
    ],
  },
]
    ...
    <StaticPageLayout title="关于蹭饭图生成器">
      ...
      <section className="space-y-2">
        ...
        <dl className="grid grid-cols-[5.5rem_1fr] gap-y-1.5 text-sm">
          ...
          <dd className="text-stone-700">v1.16.3</dd>
