...
const changelog = [
  {
    ...
    items: [
      '省份卡片文字对齐可自定义：录入弹窗中每个省份（含拆分后的每一张小卡）都有左/右对齐切换，拆分卡片可分别设置；再次点击已选中的对齐即恢复跟随所在列的默认',
      '移动端拖动卡片新增卡顿检测：连续掉帧时页面顶部提示「检测到卡顿，推荐到电脑端进行」，并指引通过首页「分享 → 在其他设备上继续此工作」把画布同步到电脑',
    ],
  },
  {
    version: 'v1.22.2',
    date: '2026-07-24 13:23',
    items: [
  ...
  },
  {
    ...
    items: [
      '新增省份卡片拆分：人多的省份可在录入弹窗中拆成多张卡片（可一键「按城市拆分」），每张卡片在地图上独立显示、独立拖动定位、各自引线到卡内同学的城市；卡内同城时标题自动显示城市名',
      '拆分后同学可在卡片之间自由分配：电脑端直接拖到另一张卡（空卡片显示「把同学拖到这里」占位），手机端用上/下按钮越过卡片边界自然进入相邻卡片；也可随时「合并为一张卡」还原',
    ],
  },
  {
    version: 'v1.21.2',
    date: '2026-07-24 02:16',
    items: [
